            ...
            try
            {
                ...
                if (_lit.Count > 0)
                {
                    ...
                    foreach (DetaNaviera item in _lit)
                    {
                        ...
                        {
                            _resultado = Convert.ToInt32(DetaNavieraDAL.ActualizarUCCL(DBComun.Estado.verdadero, item.IdDeta, User.Identity.Name,DateTime.Now.ToString("dd/MM/yyyy HH:mm"), item.t_revision, 4, item.s_marchamo));
                        }
